...
public class Logic implements LogicInterface {
    ...
    */
    ...
    public double calculate(int size, int count) {
        ...
        int countOfSimulationsWhereTwoPersonsHaveSameBirthDate = 0;
        for (int i = 0; i < count; ++i) {
            Random random = new Random(i+1);
            if (singleSimulationRun(random, size)) {
                countOfSimulationsWhereTwoPersonsHaveSameBirthDate++;
            }
        }
        return (double) countOfSimulationsWhereTwoPersonsHaveSameBirthDate/count * 100.0;
       
    }

    /**
     * This is the method that runs a single simulation on a size and return true in case simulation
     * creates a scenario where two people have the same birth date.
     */

    private boolean singleSimulationRun(Random random, int size) {
        int[] birthDatesCount = new int[365];
        int length = birthDatesCount.length;
        for (int i = 0; i < size; ++i) {
            int birthDate = random.nextInt(365);
            if (birthDatesCount[birthDate] > 0 ) {
                return true;
            }
            birthDatesCount[birthDate] = 1;
        }
        return false;
